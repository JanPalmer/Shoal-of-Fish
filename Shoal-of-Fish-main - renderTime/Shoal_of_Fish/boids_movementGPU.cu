...
__global__ void moveBoidsGPU(s_boids* boids, float deltaTime) {
	...
	centerOfMassRule(boids, index);
	separationRule(boids, index);
	alignmentRule(boids, index);

	boids->position.x[index] += boids->direction.x[index] * boids->simulationOptions.boidSpeed * deltaTime;
	boids->position.y[index] += boids->direction.y[index] * boids->simulationOptions.boidSpeed * deltaTime;
	boids->position.z[index] += boids->direction.z[index] * boids->simulationOptions.boidSpeed * deltaTime;
}