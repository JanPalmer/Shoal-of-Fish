...
inline void moveBoidsCPU(s_boids* boids, float deltaTime) {
	for (int i = 0; i < boids->count; i++) {
		centerOfMassRule(boids, i);
		separationRule(boids, i);
		alignmentRule(boids, i);

		boids->position.x[i] += boids->direction.x[i] * boids->simulationOptions.boidSpeed * deltaTime;
		boids->position.y[i] += boids->direction.y[i] * boids->simulationOptions.boidSpeed * deltaTime;
		boids->position.z[i] += boids->direction.z[i] * boids->simulationOptions.boidSpeed * deltaTime;
	}
}