...
__host__ __device__ void centerOfMassRule(s_boids* boids, int index) {
	...
	float3 centerOfMass = positionSum / (float)numberOfBoidsNearby;
...
__host__ __device__ void separationRule(s_boids* boids, int index) {
	...
	float3 repelForce = repelSum / (float)numberOfBoidsNearby;	
...
__host__ __device__ void alignmentRule(s_boids* boids, int index) {
	...
	float3 alignForce = alignSum / (float)numberOfBoidsNearby;
...
__global__ void moveBoidsGPU(s_boids* boids) {
